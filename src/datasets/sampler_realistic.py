...
class CategoriesSampler():
    ...
    def __init__(self, label, n_batch, n_cls, s_shot, q_shot, balanced, alpha = 2):
        ...
        self.alpha = alpha
        self.num_classes = max(np.array(label))
        self.n_query = self.n_cls * self.q_shot
    ...
    def __iter__(self):
        for i_batch in range(self.n_batch):
            ...
            possible_examples = []
            for c in classes:
                possible_examples += self.m_ind[c]
            query = torch.tensor(np.random.choice(possible_examples, size=self.n_query, replace=False))
